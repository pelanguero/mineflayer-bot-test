...
bot.on('chat', (username, message) => {
  ...
  if(message.startsWith("mine")){
    ...
    moveto(new GoalNear(parseInt(splitedMessge[1]), parseInt(splitedMessge[2]), parseInt(splitedMessge[3])),new Vec3( parseInt(splitedMessge[1]), parseInt(splitedMessge[2]),parseInt(splitedMessge[3])))
  }
  
})

function moveto(goal,pos){
  const defaultMove = new Movements(bot)
  bot.pathfinder.setMovements(defaultMove)
  bot.pathfinder.setGoal(goal, RANGE_GOAL)
  bot.once('goal_reached', () => {
    bot.dig(targetBlock, onDigCompleted);
  });
  //mineBlock(1,pos)
}
